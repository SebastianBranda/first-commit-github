...
const FirstCommit = () => {
  ...
  const { data, headerLink } = useFetch(repo.commits_url.split("{")[0]);

  useEffect(() => {
    const lastPageOfCommits = headerLink?.match(/[0-9]+/g).pop();
    if (lastPageOfCommits != null) {
      fetch(
        `https://api.github.com/repositories/${repo.id}/commits?page=${lastPageOfCommits}`
      )
        .then((resp) => resp.json())
        .then((myData) => {
  ...
  const renderFirstCommit = isFirstCommitLoading ? (
    "loading"
  ) : (
    <>
      ...
      <p>
        <span>Message: </span> {firstCommit.commit.message}
      </p>
      ...
      <div className="more-info">
        ...
        <a
          ...
          rel="noopener noreferrer"
        >
          <button>Go to first commit</button>
        </a>
        <Link to="/">
  ...
  return (
    <main className="container">
      <article className="info-repo">
        ...
        <p>
          <span>License: </span>
          {repo.license?.name || `Unknown`}
        </p>
        <p>
          <span>Date of creation: </span>
          {repo.created_at.split("T").shift()}
        </p>
        <p>
          <span>Last update: </span>
          {repo.updated_at.split("T").shift()}
        </p>
      </article>
